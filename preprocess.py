...
def main():
    ...
    train_cats = sorted(glob.glob(os.path.join(TRAIN_DIR, '*.tif')), key=natural_key)
    #train_dogs = sorted(glob.glob(os.path.join(TRAIN_DIR, 'dog*.tif')), key=natural_key)

    test_all = sorted(glob.glob(os.path.join(TEST_DIR, '*.tif')), key=natural_key)

    # Make the output directories
    base_out = os.path.join(BASE_DIR, 'proc{}'.format(SIZE))
    ...
    procs = dict()
    procs[1] = Process(target=resize_image, args=(train_cats, train_dir_out, ))
    procs[1].start()

    procs[3] = Process(target=resize_image, args=(test_all, test_dir_out, ))
    procs[3].start()
